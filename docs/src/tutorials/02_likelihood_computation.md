...
Regardless of the type of cognitive computational model, the way we estimate the "best" parameters begins with selecting a **criterion** to quantify the difference between observed data and data that would be generated by the hypothesized model given a specific set of parameter values. When estimating coefficients in a linear regression, for example, we choose the sum of squared differences between the observed and predicted data. The estimated parameters are those that minimize this difference (given constraints on the parameter search space) and yield predicted data most similar to the observed data.   
...
1. select evaluation criterion: likelihood, conditional RT quantiles etc.
2. select method to compute criterion: analytical solution, LAN, FPE etc.
...
Let's start with a simple example. We'll use a simplified classic DDM (not attentional) to illustrate the main points. Similar to the previous tutorial, first we'll define a model to simulate a single trial using it.
...
Similar to the example in the earlier tutorial, we find that the likelihood of observing the simulated trial is higher for the incorrect model when using a `stateStep` of .1 but the correct model has a higher likelihood when using a `stateStep` of .01.
...
With the discretization and initial state assumptions we have made, we will use the RDV sampling distribution to compute the probability of moving from one state space bin to another. Formally, the probability of being in any state bin $j$ at time $t+1$ is defined as 
...
### Relationship between the state step and the RDV sampling distribution

Going back to our example, to move from the initial state bin in the middle to one state bin higher we would need a $\triangle RDV$ that is larger than one but smaller than two state steps. With a state step of .2, this would mean a $\triangle RDV$ larger than .2 but smaller than .4. The probability of observing a $\triangle RDV$ of this size, however, is very very small with the sampling distribution defined by our model! The transition probabilities to move to any other state or over time steps would get even smaller!    

![plot](plot_2_7.png)

On the other hand, observing a $\triangle RDV$ of the same size is much more probable for the RDV sampling distribution described by the incorrect model with the larger $\sigma$ we defined above. So even though the data were generated by a model with less noise in the sampling distribution, the choice of the large state step size interferes with our ability to detect this.

![plot](plot_2_8.png)

What if we had chosen a smaller step size? If we set the `stateStep` size to 0.01 (the default in ADDM.jl) and keep the `timeStep` as 100, as we did above for illustration purposes, the discretized space would look like the left figure below. Since it is a difficult to see effect on the y-axis we zoom in to show how decreasing the `stateStep` divides the RDV axis more finely. This affects the likelihood computation by decreasing the size of change required to move from one state space bin to another. In this case, the size of the change we would need to observe would be larger than 0.01 but smaller than 0.02. As we did before, we compute the probability of observing a change of this size using the RDV sampling distribution. As depicted in the right figure below, this smaller change has a much higher probability of being observed when sampling from the RDV distribution than the larger change needed with the large state step size (.123 vs 2.88e-11).

![plot](plot_2_9.png)

More importantly, this probability is also larger to be observed from the true probability distribution compared to the incorrect distribution with the larger noise.

![plot](plot_2_10.png)

### Propogation of the RDV distribution and likelihood computation

So far, we've looked at the RDV probability distribution at the first timestep. Specifically, we operationalized it as a distribution with all its mass exactly in the middle of the two boundaries because our models have a bias value of 0. Additionally, we introduced the equation describing the probability of being in each state bin at each time step. Specifically, to compute the probability of being in a state at a timestep, we sum the products of being in any state at the previous time step and the transition probability of moving to the state we are interested in.

To compute the likelihood, however, we want to know the probability of crossing the boundary associated with the observed choice at the timestep that maps onto the observed response time. In other words, the transition we are interested in is not one between states *within* the two boundaries, but from any given state to a state *outside* the boundaries. With this key difference in mind, we can use the same principle as before to compute the likelihood. 

For our example where the observed choice of "Left" is associated with the upper boundary, this corresponds to the sum of the probability of being in any state $i$ at time step right before the observed RT times the probability of crossing the upper boundary from state $i$.

$$p^{UP}_{RT} = \sum_{i} p^i_{RT-1} \times p^{i \rightarrow UP}_{RT-1}$$


To compute $p^i_{RT-1}$, first we need to propogate the probability distribution from the initial state up to $RT-1$ in time.

Equation for boundary crossing  
Probability of boundary crossing with small and large statestep for correct and incorrect models - grid of heatmaps     
