...
Similar to the example in earlier tutorial, we find that the likelihood of observing the simulated trial is higher for the incorrect model when using a `stateStep` of .1 but the correct model has a higher likelihood when using a `stateStep` of .01.
