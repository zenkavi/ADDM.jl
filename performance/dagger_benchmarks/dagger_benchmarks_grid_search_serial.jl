...
@everywhere function grid_search_serial_thread(data, param_grid, likelihood_fn = nothing, 
  ...
  for cur_grid_params in param_grid
    ...
    # Trial likelihoods will be a dict indexed by trial numbers
      all_nll[cur_grid_params], trial_likelihoods[cur_grid_params] = compute_trials_nll_threads(model, data, likelihood_fn, likelihood_args; 
      ...
      all_nll[cur_grid_params] = compute_trials_nll_threads(model, data, likelihood_fn, likelihood_args, sequential_model = sequential_model)
    end
